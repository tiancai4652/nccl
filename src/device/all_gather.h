...
namespace
{
  ...
  {
    printf("******ncclShmem.comm.dimension: %d\n", ncclShmem.comm.dimension);
    const int channelId = ncclShmem.channelId;
    bool isXdimension = (channelId % 2 == 0);
    bool isXdimensionAlg=ncclShmem.comm.dimension == 0;
    if (isXdimension!=isXdimensionAlg)
    {
      return;
    }
    ...
    int userRanksX[x];
    int userRanksY[y];
    int *ringRanks = isXdimension ? userRanksX : userRanksY;
    // nranks
    int nranks = isXdimension ? x : y;
    // ringRanks
    {
      int x_coord = myRank % x;
      int y_coord = myRank / x;
      ...
      if (isXdimension)
      {
        ...
        for (int i = 0; i < x; ++i)
        {
          int next_x = (x_coord + i) % x;
          int next_rank = y_coord * x + next_x;
          ringRanks[i] = next_rank;
        }
      }
      else
      {
        ...
        for (int i = 0; i < y; ++i)
        {
          int next_y = (y_coord + i) % y;
          int next_rank = next_y * x + x_coord;
          ringRanks[i] = next_rank;
        }
      }
    }
    ...
    // print
    if (tid == 0 && is_print) {
      printf("*******myRank = %d, isXdimension = %d\n", myRank, isXdimension ? 1 : 0);
    ...
    if (tid < workNthreads)
    {
      ...
      Primitives<T, RedOp, FanSymmetric<1>, 1, Proto, 0, isNetOffload> prims(tid, workNthreads, &ring->prev, &ring->next, inputBuf, outputBuf, work->redOpArg, 0, 0, 0, work, NULL, isNetOffload ? NCCL_MAX_NET_SIZE : 0);
    ...
    else if (inputBuf != outputBuf + ringRanks[0] * count)
    {
      ...
      reduceCopy<COLL_UNROLL, RedOp, T, 0, 1, 1, 0, 1, 1, /*PreOpSrcs=*/0>(tid - workNthreads, nthreads - workNthreads, work->redOpArg, &work->redOpArg, false, 1, (void **)&inputBuf, 1, (void **)&outputBuf, partCount);
    }
...
struct RunWorkColl<ncclFuncAllGather, T, RedOp, NCCL_ALGO_PAT, NCCL_PROTO_SIMPLE>
{
  __device__ __forceinline__ void run(int tid, int nthreads, struct ncclDevWorkColl *work)
  {
    ...
    { // Worker threads
      ...
      Primitives<T, RedOp, FanSymmetric<1>, 0, Proto, 0> prims(tidInGroup, groupSize, (int *)shmem->recvDims, (int *)shmem->sendDims, inputBuf, outputBuf, work->redOpArg, group, 0, 0, nullptr, nullptr, 0, primsModePatAg);
      ...
      while (1)
      {
        ...
        if (tidInGroup == 0)
          poll.store(0, cuda::memory_order_release); // Return element to compute thread
        if (last)
          break;
        step += nGroups;
      }
        ...
        while (railAllOffset < railAllSize)
        {
          ...
          if (nSrcs != 0 && outIsDst + nDsts != 0)
          {
            reduceCopy<ncclCollUnroll(), RedOp, T,
                       ...
                       /*PreOpSrcs=*/0>(tid, tn, 0, nullptr, false,
                                        ...
                                        { return d < outIsDst    ? outbuf + userOneBeg
                                                 : work->regUsed ? (char *)dstPtrs[d - outIsDst] + userOneBeg
                                                                 : (char *)dstPtrs[d - outIsDst] + railAllOffset; }, delta);
          }
          railAllOffset += delta;
          node += 1;
        }
